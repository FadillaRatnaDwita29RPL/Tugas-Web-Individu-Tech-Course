<?php include './koneksi.php';

?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=, initial-scale=1.0">
    <link rel="stylesheet" type="text/css" href="profile.css">
    <link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css">
    <title>Profile</title>
</head>
<body>

    <?php 
        session_start();
        $qry=mysqli_query($conn,"select * from admin where email = '".$_SESSION['email']."'");
        $dt_user=mysqli_fetch_array($qry);

    ?>
    <div class="container">
        <div class="bg">
        <div class="button">
                
                <a href="profile-admin.php"><button class="edit" type="submit">EDIT</button></a>
                <a href="delete.php"><button class="out" onclick="return confirm('Are you sure to delete your account?')">DELETE</button></a>
            </div>
        </div>
        
        <div class="prof">
            <div class="flip-card">
                    <div class="flip-card-inner">
                        <div class="flip-card-front">
                        <?php if(!$dt_user['photo']) {?>
                            <img src="usericon.jpg" alt="Profile" class="profil">
                        <?php }else{ ?>
                            <img src="<?php echo "file/".$dt_user['photo']; ?>" alt="Profile User" class="profil">
                        <?php } ?>
                        </div>
                        <div class="flip-card-back">
                            <div class="text">
                                <?php if(!$dt_user['photo']) {?>
                                    <a href="add-profile-admin.php"><img src="round-add-button.png" alt="ADD BUTTON" width="64px" height="64px"></a>
                                <?php }else{ ?>
                                    <a href="delete-photo-admin.php"  onclick="return confirm('Are you sure to delete your photo profile?')"><img src="trash.png" alt="ADD BUTTON" width="64px" height="64px"></a>
                                <?php } ?>
                            </div>
                        </div>
                    </div>
            </div>
          
            <div class="identity">
                <h3 class="name"><?=$dt_user['name']?></h3>
            </div>
        </div>
    </div>

    <div class="profile">
        <div class="quest">
            <h3>ID Admin</h3>   
            <h3>Age</h3>
            <h3>Gender</h3>
            <h3>Address</h3>
            <h3>Date of Birth</h3>
            <h3>Phone Number</h3>
            <h3>Email</h3>
            <h3>Password</h3>
        </div>
        
        <div class="form">
            <input type="text" name="id_user" value="<?=$dt_user['id_admin']?>" readonly>
            <br>
            <input type="number" name="age" value="<?=$dt_user['age']?>" readonly>
            <br>
            <input type="text" name="gender" value="<?=$dt_user['gender']?>" readonly>
            <br>
            <input type="text" name="address" value="<?=$dt_user['address']?>" readonly>
            <br>
            <input type="date" name="date_birth" value="<?=$dt_user['date_birth']?>" readonly>
            <br>
            <input type="text" name="phone_number" value="<?=$dt_user['phone_number']?>" readonly>
            <br>
            <input type="email" name="email" value="<?=$dt_user['email']?>" readonly>
            <br>
            <input type="password" name="password" value="<?=$dt_user['password']?>" readonly>
            <br>
        </div>
    </div>

    <div class="sosbar">
        <a href="index-admin.php"><i class="fas fa-home"></i></a>
    </div>

   
</body>
</html>